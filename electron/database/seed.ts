...
import { getCurrentTimestamp, formatDate } from './connection'
...
export function seedDatabase(db: Database.Database): void {
  ...
  // Income categories
  insertCategory.run('Satış Geliri', 'income', null, 1, now, now)
  insertCategory.run('Hizmet Geliri', 'income', null, 1, now, now)
  insertCategory.run('Faiz Geliri', 'income', null, 1, now, now)
  insertCategory.run('Diğer Gelirler', 'income', null, 1, now, now)

  // Expense categories
  insertCategory.run('Personel Gideri', 'expense', null, 1, now, now)
  insertCategory.run('Kira Gideri', 'expense', null, 1, now, now)
  insertCategory.run('Elektrik/Su/Doğalgaz', 'expense', null, 1, now, now)
  insertCategory.run('İletişim Gideri', 'expense', null, 1, now, now)
  insertCategory.run('Ofis Malzemeleri', 'expense', null, 1, now, now)
  insertCategory.run('Ulaşım Gideri', 'expense', null, 1, now, now)
  insertCategory.run('Pazarlama Gideri', 'expense', null, 1, now, now)
  insertCategory.run('Yazılım/Lisans', 'expense', null, 1, now, now)
  insertCategory.run('Diğer Giderler', 'expense', null, 1, now, now)
  ...
  insertParty.run('customer', 'ABC Teknoloji A.Ş.', '1234567890', '0212 123 45 67', 'info@abcteknoloji.com', 'İstanbul, Türkiye', 'Yazılım geliştirme müşterisi', now, now)
  insertParty.run('customer', 'XYZ Danışmanlık Ltd.', '9876543210', '0216 987 65 43', 'iletisim@xyzdanismanlik.com', 'Ankara, Türkiye', 'Danışmanlık hizmetleri', now, now)
  insertParty.run('vendor', 'Tech Solutions Inc.', '5555555555', '0312 555 55 55', 'contact@techsolutions.com', 'İzmir, Türkiye', 'Yazılım lisans tedarikçisi', now, now)
  insertParty.run('vendor', 'Ofis Malzemeleri A.Ş.', '6666666666', '0232 666 66 66', 'satis@ofismalz.com', 'Bursa, Türkiye', 'Ofis malzemeleri tedarikçisi', now, now)
  insertParty.run('other', 'Freelance Geliştirici', null, '0533 111 22 33', 'developer@email.com', null, 'Dış kaynak geliştirici', now, now)
  ...
  for (let i = 0; i < 7; i++) {
    ...
    insertRate.run(dateStr, 'TRY', 'USD', 32.50 + Math.random() * 0.5, 'tcmb', now)
    // EUR rates (approximately)
    insertRate.run(dateStr, 'TRY', 'EUR', 35.20 + Math.random() * 0.5, 'tcmb', now)
  }
  ...
  const project1Id = insertProject.run(
    1, 'Web Uygulaması Geliştirme', 150000, 'TRY',
    formatDate(new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000)),
    formatDate(new Date(today.getTime() + 60 * 24 * 60 * 60 * 1000)),
    'active', 'E-ticaret web uygulaması', now, now
  ).lastInsertRowid

  const project2Id = insertProject.run(
    2, 'Danışmanlık Projesi', 25000, 'USD',
    formatDate(new Date(today.getTime() - 60 * 24 * 60 * 60 * 1000)),
    formatDate(new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000)),
    'active', 'IT altyapı danışmanlığı', now, now
  ).lastInsertRowid
  ...
  insertMilestone.run(project1Id, 'Analiz ve Tasarım', formatDate(new Date(today.getTime() - 20 * 24 * 60 * 60 * 1000)), 30000, 'TRY', 'completed', 'Tamamlandı', now, now)
  insertMilestone.run(project1Id, 'Backend Geliştirme', formatDate(new Date(today.getTime() + 10 * 24 * 60 * 60 * 1000)), 50000, 'TRY', 'pending', 'Devam ediyor', now, now)
  insertMilestone.run(project1Id, 'Frontend Geliştirme', formatDate(new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000)), 50000, 'TRY', 'pending', null, now, now)
  ...
  insertMilestone.run(project2Id, 'İhtiyaç Analizi', formatDate(new Date(today.getTime() - 45 * 24 * 60 * 60 * 1000)), 10000, 'USD', 'completed', null, now, now)
  ...
  // Income transactions
  insertTransaction.run('income', 1, 2, Number(project1Id), null, formatDate(new Date(today.getTime() - 25 * 24 * 60 * 60 * 1000)), 30000, 'TRY', 20, 6000, 0, 0, 36000, 'Analiz teslimi için ödeme', 'INV-001', 1, now, now)
  insertTransaction.run('income', 2, 2, Number(project2Id), null, formatDate(new Date(today.getTime() - 40 * 24 * 60 * 60 * 1000)), 10000, 'USD', 0, 0, 0, 0, 10000, 'İhtiyaç analizi ödemesi', 'INV-002', 1, now, now)
  insertTransaction.run('income', 1, 1, null, null, formatDate(new Date(today.getTime() - 15 * 24 * 60 * 60 * 1000)), 15000, 'TRY', 20, 3000, 0, 0, 18000, 'Ek modül satışı', 'INV-003', 1, now, now)
  ...
  insertTransaction.run('expense', null, 5, null, null, formatDate(new Date(today.getTime() - 20 * 24 * 60 * 60 * 1000)), 25000, 'TRY', 0, 0, 0, 0, 25000, 'Ocak ayı maaşları', 'PAY-001', 1, now, now)
  insertTransaction.run('expense', null, 6, null, null, formatDate(new Date(today.getTime() - 18 * 24 * 60 * 60 * 1000)), 8000, 'TRY', 20, 1600, 0, 0, 9600, 'Ofis kirası', 'RENT-001', 1, now, now)
  insertTransaction.run('expense', 3, 12, null, null, formatDate(new Date(today.getTime() - 10 * 24 * 60 * 60 * 1000)), 500, 'USD', 0, 0, 0, 0, 500, 'Yazılım lisansı', 'LIC-001', 1, now, now)
  insertTransaction.run('expense', null, 7, null, null, formatDate(new Date(today.getTime() - 8 * 24 * 60 * 60 * 1000)), 1200, 'TRY', 20, 240, 0, 0, 1440, 'Elektrik faturası', 'UTIL-001', 1, now, now)
  ...
  insertTransaction.run('expense', null, 8, null, null, formatDate(new Date(today.getTime() - 2 * 24 * 60 * 60 * 1000)), 350, 'TRY', 20, 70, 0, 0, 420, 'İnternet faturası', 'NET-001', 1, now, now)
  ...
  const debt1Id = insertDebt.run(
    ...
    'open', 'Yazılım tedarikçisi borcu', now, now
  ).lastInsertRowid
  ...
  const debt2Id = insertDebt.run(
    ...
    'open', 'Proje taksitli ödeme', now, now
  ).lastInsertRowid
  ...
  const debt3Id = insertDebt.run(
    ...
    'open', 'Danışmanlık alacağı', now, now
  ).lastInsertRowid
  ...
  insertInstallment.run(debt1Id, formatDate(new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000)), 3000, 'TRY', 'paid', 3000, 'Ödendi', now, now)
  ...
  insertInstallment.run(debt2Id, formatDate(new Date(today.getTime())), 10000, 'TRY', 'partial', 5000, 'Kısmi ödeme', now, now)
